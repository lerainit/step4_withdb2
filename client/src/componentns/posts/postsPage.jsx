  ...
  return (
    <>
      <div className={styles.post_page_container} >
        ...
        dispatch({type:setProducts})}}>Show more</button><span className={styles.posts_span}>Likes:{products[index].likes}</span><svg onClick={async() => {
            await dispatch(setCounterAC())
            dispatch(incrementLikesAC({ index: index, userIndex: userIndex ,counter:productsArr}))
          ...
          <CommentsForm index={index} userIndex={userIndex} userId ={userId} /></div>)
