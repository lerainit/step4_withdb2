...
const authIndex = users.findIndex(({isAuth}) => isAuth === true)
  ...
  {    users.map(({url,name,id,isAuth},index) => isAuth ? <NavLink className={styles.cart_link} to ={`/${name.replace(/\s+/g, '')}`} ><img className={styles.auth_img} src={url} alt="img" /><h2 className={styles.auth_name}>{name}</h2></NavLink> : null)
              ...
              <h4>Following</h4>
            { users.map(({url,name,id,isAuth,isFollower},index) =>  isFollower ? <div className={styles.follower_container} ><NavLink className={styles.cart_link} to ={`/${name.replace(/\s+/g, '')}`} ><img className={styles.cart} src={url} alt="img" /><h2 className={styles.user_name}>{name}</h2></NavLink><button onClick={
                async() => {
